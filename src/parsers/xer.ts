...
export async function parseXer(file: File): Promise<ProjectData> {
  ...
  for (const line of lines) {
    if (line.startsWith('%T') && line.includes('TASK')) {
      ...
      const id = row['task_id'] || row['act_id'] || row['task_code'] || row['task_id'] || cryptoRandomId();
      const name = row['task_name'] || row['task_name'] || row['act_name'] || 'Activity';
      const start = row['early_start_date'] || row['start_date'] || row['act_start_date'] || '';
      const finish = row['early_end_date'] || row['finish_date'] || row['act_end_date'] || '';
